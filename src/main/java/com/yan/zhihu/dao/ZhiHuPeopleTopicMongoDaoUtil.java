...
public class ZhiHuPeopleTopicMongoDaoUtil {
	...
	public List<ZhiHuPeopleTopic> findZhiHuPeopleTopicDocumentsByCondition(Map<String, Object> condition){
		List<ZhiHuPeopleTopic> zhiHuPeopleTopics = null;
		...
		if(condition != null && condition.size() > 0) {
			...
			if(docs != null){
				zhiHuPeopleTopics = new ArrayList<ZhiHuPeopleTopic>();
				
				for(Document doc : docs){
					ZhiHuPeopleTopic zhiHuPeopleTopic = new ZhiHuPeopleTopic();
					//将document转换为interview
					//System.out.println(doc.get("_id"));
					zhiHuPeopleTopic = (ZhiHuPeopleTopic)SchameDocumentUtil.documentToSchame(doc, ZhiHuPeopleTopic.class);
					
					zhiHuPeopleTopics.add(zhiHuPeopleTopic);
				}
			}
		}
		
		return zhiHuPeopleTopics;
	}
	
	public Long countZhiHuPeopleTopicVoDocumentsByCondition(Map<String, Object> condition){
		long count = 0L;
	...
	public ZhiHuPeopleTopic findZhiHuPeopleTopicByUserIdAndToken(String userId, String token) {
		ZhiHuPeopleTopic zhiHuPeopleTopic = null;
		if(userId!= null && !"".equals(userId.trim())) {
			...
			bsons.add(Filters.eq("token", token));
